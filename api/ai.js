...
- 嚴格控制篇幅，必須能在手機螢幕上一眼看完（盡量不超過 200 字）。
...
5. 【觀察點】多方確認條件 + 空方風險價位，各一行
- **支撐／壓力**：支撐位 [價格] ／ 壓力位 [價格]
- **觀察點位**：[例如：若站穩 XX 價位可留意分批機會 ／ 跌破 XX 價位需注意風險控制]
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 600, // 200 字主回應 + 中間思考預留；output 按實際 token 計費，cap 只防失控
